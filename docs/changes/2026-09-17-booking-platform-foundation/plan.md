...
| 2026-09-18 | El PR #2 se mergea como administrador con el check `harness` en rojo | GitHub no permite autoaprobarse y solo hay un mantenedor; los recibos locales de `sdlc approve` quedan como prueba de aprobación y la verificación en plataforma queda pendiente de un segundo revisor |
| 2026-09-18 | Fase release: la IaC de ADR-0005 sale de este cambio y pasa a un registro propio; aquí solo se publican los artefactos firmados | Decisión de huanrasan: acotar la release al artefacto y tratar la infraestructura como su propio cambio, con su spec y su costo |
| 2026-09-18 | Dos imágenes (`runner` y `migrator`) en lugar de una | El CLI de Prisma arrastraba Studio, pglite, effect y TypeScript (~200 MB) a lo que corre en producción; separarlas baja la imagen de ejecución de 1,02 GB a 819 MB y reduce su superficie |
